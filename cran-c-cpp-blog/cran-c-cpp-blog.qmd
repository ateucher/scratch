...
The R package landscape is dynamic one, with changes in infrastructure common, especially when CRAN makes changes to their policies and requirements. This is particularly true for packages that include low-level compiled code, requiring developers to be nimble in responding to these changes.
...
## 1. NOTE regarding `SystemRequirements: C++11`

Many package authors might have noticed a new NOTE on R-devel when submitting to CRAN a package containing C++ code:
...
After making these changes, the package should install without trouble on R 3.6 and above. However, on R 3.5 and below, there may be systems with very old compilers where it won't build (RHEL 7 is one such system). In practice, the vast majority of users will have a newer version of R installed, so in most cases it should be fine to just support R \>= 3.6.2 in your package. If you want to be confident that your package will be installable on R 3.5 and below with old compilers, there are several options; we offer two of the simplest approaches here:

-   You can use a configure script at the top level of the package, and have it add `CXX_STD=CXX11` for R 3.5 and below. An example (unmerged) [pull request to the readxl](https://github.com/tidyverse/readxl/pull/722/files) package demonstrates this approach.
...
-->
```
## 2. WARNING regarding the use of `sprintf()` in C/C++

Another recent change in CRAN checks on R-devel that authors might encounter is the disallowing of the use of the C functions `sfprint()` and `vsprinf()`. R CMD check on R-devel may throw warnings that look something like this:
...
## 3. WARNING regarding the use of strict prototypes in C
...
This comes from C function declarations that look like this, with no arguments specified (which is very common):
...
The ideal solution in this case would be to submit patches to the upstream libraries so you don't have to deal with the ongoing maintenance of you local patches, but that is not always possible.

The `-Wstrict-prototypes` compiler flag will also catch deprecated function definitions of the form:
...
where the argument types are not declared. This is much less common than the case of not specifying any arguments, and is solved by declaring the types of the arguments:
